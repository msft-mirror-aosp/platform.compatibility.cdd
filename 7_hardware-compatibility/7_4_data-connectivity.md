...
*   [C-SR] Are STRONGLY RECOMMENDED to notify the user that answering an
    ...
    which indicates to the user that answering an incoming call will cause
...
*   [C-1-6] Are STRONGLY RECOMMENDED, when the [`ConnectivityManager.reportNetworkConnectivity()`](
    ...
    API method is called, to re-evaluate the Internet access on the `Network` and,
